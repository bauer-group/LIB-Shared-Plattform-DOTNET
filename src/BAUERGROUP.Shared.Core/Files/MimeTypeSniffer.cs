...
namespace BAUERGROUP.Shared.Core.Files
{
    ...
    public static class MimeTypeSniffer
    {
        ...
        private static readonly List<MimeSignature> Signatures = new()
        {
            ...
            new MimeSignature("image/webp", new byte[] { 0x52, 0x49, 0x46, 0x46 }, 0, new byte[] { 0x57, 0x45, 0x42, 0x50 }, 8), // RIFF....WEBP
            ...
            new MimeSignature("image/svg+xml", System.Text.Encoding.UTF8.GetBytes("<?xml"), additionalCheck: data => ContainsString(data, "<svg")),
            new MimeSignature("image/avif", new byte[] { 0x00, 0x00, 0x00 }, 0, new byte[] { 0x66, 0x74, 0x79, 0x70, 0x61, 0x76, 0x69, 0x66 }, 4), // ftyp avif
            new MimeSignature("image/heic", new byte[] { 0x00, 0x00, 0x00 }, 0, new byte[] { 0x66, 0x74, 0x79, 0x70, 0x68, 0x65, 0x69, 0x63 }, 4), // ftyp heic
            ...
            new MimeSignature("audio/wav", new byte[] { 0x52, 0x49, 0x46, 0x46 }, 0, new byte[] { 0x57, 0x41, 0x56, 0x45 }, 8), // RIFF....WAVE
            ...
            // Video
            new MimeSignature("video/mp4", new byte[] { 0x00, 0x00, 0x00 }, 0, new byte[] { 0x66, 0x74, 0x79, 0x70 }, 4), // ftyp
            ...
            new MimeSignature("video/avi", new byte[] { 0x52, 0x49, 0x46, 0x46 }, 0, new byte[] { 0x41, 0x56, 0x49, 0x20 }, 8), // RIFF....AVI
            new MimeSignature("video/x-flv", new byte[] { 0x46, 0x4C, 0x56 }), // FLV
            new MimeSignature("video/quicktime", new byte[] { 0x00, 0x00, 0x00 }, 0, new byte[] { 0x6D, 0x6F, 0x6F, 0x76 }, 4), // moov
            new MimeSignature("video/quicktime", new byte[] { 0x00, 0x00, 0x00 }, 0, new byte[] { 0x66, 0x72, 0x65, 0x65 }, 4), // free
            ...
            // Data formats
            new MimeSignature("application/json", new byte[] { 0x7B }, additionalCheck: IsJson), // { with validation
            ...
            new MimeSignature("text/html", System.Text.Encoding.UTF8.GetBytes("<!DOCTYPE html"), stringComparison: StringComparison.OrdinalIgnoreCase),
            new MimeSignature("text/html", System.Text.Encoding.UTF8.GetBytes("<html"), stringComparison: StringComparison.OrdinalIgnoreCase),
            new MimeSignature("text/html", System.Text.Encoding.UTF8.GetBytes("<head"), stringComparison: StringComparison.OrdinalIgnoreCase),
            new MimeSignature("text/html", System.Text.Encoding.UTF8.GetBytes("<body"), stringComparison: StringComparison.OrdinalIgnoreCase),
        };
        ...
        private sealed class MimeSignature
        {
            ...
            public MimeSignature(
                string mimeType,
                byte[] signature,
                int offset = 0,
                byte[]? secondSignature = null,
                int secondOffset = 0,
                Func<byte[], bool>? additionalCheck = null,
